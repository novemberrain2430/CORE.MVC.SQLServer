...
````bash
Add-Migration Your_Migration_Name -Context SQLServerDbContext
````

Using EF Core command line tool;

````bash
dotnet ef migrations add Your_Migration_Name --context SQLServerDbContext
````
...
````bash
Add-Migration Your_Migration_Name -Context SQLServerTenantDbContext -OutputDir TenantMigrations
````

Using EF Core command line tool:

````bash
dotnet ef migrations add Your_Migration_Name --context SQLServerTenantDbContext --output-dir TenantMigrations
````
